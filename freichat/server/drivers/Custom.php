...
$query="select distinct f.status_mesg,f.username,f.session_id,f.status,f.guest,f.in_room from frei_session AS f,user_request,users_list where user_request.request_for='mentor' AND user_request.status='accepted' AND users_list.id=user_request.frm_usr AND f.session_id!=users_list.id AND f.time>".$this->online_time2 . " AND f.status!=2 AND f.status!=0  AND f.session_id!=" . $_SESSION[$this->uid . 'usr_ses_id'] . "";
...
//------------------------------------------------------------------------------     
    public function get_users() {
        ...
        //CUSTOM_USERS_QUERY_START
            $query = "SELECT DISTINCT f.status_mesg,f.username,f.session_id,f.status,f.guest,f.in_room
                   FROM frei_session AS f,user_request,users_list
                  WHERE f.time>" . $this->online_time2 . "
                   AND f.session_id!=" . $_SESSION[$this->uid . 'usr_ses_id'] . "
                   AND f.status!=2
                   AND f.status!=0
                   AND f.guest=0 AND f.session_id = users_list.id
AND users_list.email = user_request.to_user
AND user_request.request_for ='mentor' ";
